...
// UploadGroupImage POST /api/group-chats/upload
// 接收 multipart/form-data 单文件，返回 {"url":"/api/uploads/xxx.png"}
// 单文件最大 10MB；仅图片类型
func UploadGroupImage(c *gin.Context) {
	...
	dir := uploadsDir()
	name := fmt.Sprintf("g_%d_%s%s", time.Now().UnixNano(), uuid.New().String()[:8], ext)
