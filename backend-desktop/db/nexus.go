...
type NexusSettings struct {
	...
	SignalingSecret string `json:"signaling_secret,omitempty"` // deprecated, kept for DB compat
}

func GetNexusSettings() (*NexusSettings, error) {
	var s NexusSettings
	var visible, wanEnabled int
	var sigURL, sigSecret sql.NullString
	err := DB.QueryRow(`SELECT visible, nickname, listen_port, COALESCE(signaling_url,''), COALESCE(wan_enabled,0), COALESCE(signaling_secret,'') FROM nexus_settings WHERE id=1`).
		Scan(&visible, &s.Nickname, &s.ListenPort, &sigURL, &wanEnabled, &sigSecret)
	if err != nil {
		return &NexusSettings{
			Visible:      true,
			ListenPort:   3001,
			WANEnabled:   true,
			SignalingURL:  "wss://lingxi-singaling-server.onrender.com/ws",
		}, nil
	}
	s.Visible = visible == 1
	s.WANEnabled = wanEnabled == 1
	if sigURL.Valid {
		s.SignalingURL = sigURL.String
	}
	return &s, nil
}
...
func UpdateNexusSettings(s *NexusSettings) error {
	...
	_, err := DB.Exec(`UPDATE nexus_settings SET visible=?, nickname=?, listen_port=?, signaling_url=?, wan_enabled=?, signaling_secret='' WHERE id=1`,
		v, s.Nickname, s.ListenPort, s.SignalingURL, w)
	return err
}
...
func GetNexusInstanceID() string {
	var id string
	err := DB.QueryRow(`SELECT value FROM kv_store WHERE key='nexus_instance_id'`).Scan(&id)
	if err != nil {
		return ""
	}
	return id
...
func SetNexusInstanceID(id string) {
	DB.Exec(`INSERT OR REPLACE INTO kv_store (key, value) VALUES ('nexus_instance_id', ?)`, id)
}
...
func CreateA2AConversation(c *A2AConversation) (int64, error) {
	...
	res, err := DB.Exec(`
		INSERT INTO a2a_conversations
			(conv_uuid, local_agent_id, remote_agent_name, remote_peer_id, remote_peer_nickname,
			 topic, goal, initial_prompt, max_rounds, status, require_approval, initiated_by, deadline, remote_conv_id)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		c.ConvUUID, c.LocalAgentID, c.RemoteAgentName, c.RemotePeerID, c.RemotePeerNickname,
...
func GetA2AConversationByUUID(convUUID string) (*A2AConversation, error) {
	var c A2AConversation
	var reqAppr int
	var convUUIDVal sql.NullString
	err := DB.QueryRow(`
		SELECT id, COALESCE(conv_uuid,''), local_agent_id, remote_agent_name, remote_peer_id, remote_peer_nickname,
		...
		FROM a2a_conversations WHERE conv_uuid=? AND conv_uuid!='' ORDER BY id DESC LIMIT 1`, convUUID).
		Scan(&c.ID, &convUUIDVal, &c.LocalAgentID, &c.RemoteAgentName, &c.RemotePeerID, &c.RemotePeerNickname,
	...
	if convUUIDVal.Valid {
		c.ConvUUID = convUUIDVal.String
	}
	return &c, nil
}
...
func GetA2AConversationByRemoteID(remoteConvID int64) (*A2AConversation, error) {
	var c A2AConversation
	var reqAppr int
	var convUUID sql.NullString
	err := DB.QueryRow(`
		SELECT id, COALESCE(conv_uuid,''), local_agent_id, remote_agent_name, remote_peer_id, remote_peer_nickname,
		       topic, goal, initial_prompt, max_rounds, current_round, status, require_approval,
		       summary, decisions_json, initiated_by, remote_conv_id, local_session_id, deadline, created_at, updated_at
		FROM a2a_conversations WHERE remote_conv_id=? AND remote_conv_id!=0 ORDER BY id DESC LIMIT 1`, remoteConvID).
		Scan(&c.ID, &convUUID, &c.LocalAgentID, &c.RemoteAgentName, &c.RemotePeerID, &c.RemotePeerNickname,
			&c.Topic, &c.Goal, &c.InitialPrompt, &c.MaxRounds, &c.CurrentRound, &c.Status, &reqAppr,
			&c.Summary, &c.DecisionsJSON, &c.InitiatedBy, &c.RemoteConvID, &c.LocalSessionID, &c.Deadline, &c.CreatedAt, &c.UpdatedAt)
	if err != nil {
		return nil, err
...
func ListA2AConversations() ([]A2AConversation, error) {
	...
	for rows.Next() {
		...
		var reqAppr int
		var convUUID sql.NullString
		if err := rows.Scan(&c.ID, &convUUID, &c.LocalAgentID, &c.RemoteAgentName, &c.RemotePeerID, &c.RemotePeerNickname,
			&c.Topic, &c.Goal, &c.InitialPrompt, &c.MaxRounds, &c.CurrentRound, &c.Status, &reqAppr,
...
func DeleteA2AConversation(id int64) error {
	_, err := DB.Exec(`DELETE FROM a2a_conversations WHERE id=?`, id)
	return err
}
