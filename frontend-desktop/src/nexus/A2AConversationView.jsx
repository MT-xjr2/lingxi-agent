...
export default function A2AConversationView({ convId, onBack }) {
  ...
  const displayItems = useMemo(() => {
    ...
    // 己方 Agent 正在流式输出
    if (a2aIsStreaming && a2aLiveBlocks.length > 0) {
      items.push({ type: 'local_live', liveBlocks: a2aLiveBlocks });
    } else if (a2aIsStreaming && a2aLiveBlocks.length === 0) {
      items.push({ type: 'local_connecting' });
    }

    // 对方 Agent 正在流式输出（跨实例转发）
    ...
    return items;
      ...
      conv?.local_session_id, localAgentName, remoteAgentName]);
